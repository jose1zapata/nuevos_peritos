...
void __fastcall TForm11::Image1Click(TObject *Sender)
{
        ...
        if(!Edit1->Text.IsEmpty()){
            ...
            if(total>0){
                ...
                Form1->Label61->Caption=Query1->FieldByName("numeroplanilla")->Value;
                ...
                valor="select * from observaciones where avaluo_id='"+Edit1->Text+"'";
                Query1->Close();
                Query1->SQL->Clear();
                Query1->SQL->Add(valor);
                Query1->Active=true;
                Form1->Memo1->Lines->Add(Query1->FieldByName("nota")->Value);
                Form11->Close();
            }else{
